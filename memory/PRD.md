...
### Sep 2026 (cont. 8)
- ✅ **Fixed: Approved permission hours now reduce the 8h clock-out requirement**: Root cause — `/attendance/clock-out`'s hard 8h block (added in a prior session) only excluded break/pause time, never checked the `permissions` table, so an employee who took approved permission (e.g. 1h doctor visit) still had to work a full 8h before being allowed to clock out. New `get_approved_permission_minutes(user_id, date)` helper sums today's `status='approved'` permission minutes; `required_minutes = max(0, 8h - permission_minutes_today)` is now used for both the clock-out block and the `is_short_day` flag (so it also stops incorrectly triggering the 3-short-days half-leave deduction). `/attendance/status` now returns `permission_minutes_today` + `effective_required_hours` so the frontend timer reflects the reduced target live. `TimeTrackerCard.jsx` (shared by Employee + Manager) now uses `effective_required_hours` instead of a hardcoded `8` in the countdown text, progress bar, and short-day color, plus shows a note when the requirement was reduced due to permission. Verified via curl: 7h05m worked + 60min approved permission → clock-out succeeds (`is_short_day:false`); 7h05m worked with no permission → still correctly blocked (`400`, "0h 55m more needed").
...
### Sep 2026 (cont. 10)
- ✅ **Emergency Clock-Out**: For sudden emergencies that can't be pre-planned via Permission/Half-Day Leave, employees & managers now have an "Emergency Clock-Out" link (shown only when currently short of the required hours) on `TimeTrackerCard.jsx`. Opens a dialog requiring a reason (min 5 chars); submits `emergency_reason` in the existing `/attendance/clock-out` call. Backend: bypasses the hard 8h block entirely when `emergency_reason` is provided, sets new `is_emergency_clockout`/`emergency_reason` columns on `attendance`, **exempts it from the "3 short days → 0.5 leave deducted" penalty** (`check_and_deduct_half_day_leave` skipped when `is_emergency`), and immediately emails the employee's reporting manager (`_emergency_clockout_email_html`, same mocked-SMTP pattern as CR notifications) + surfaces a live count in the `/admin/notifications` bell (`type:"emergency"`). Admin/Manager "Attendance" table now shows a red "Emergency" badge with the reason as a tooltip/caption instead of "Short Day". Verified via curl: normal clock-out at 2h → blocked (400); emergency clock-out at 2h with reason → succeeds (200, `is_emergency_clockout:true`, `is_short_day:true` for visibility only, penalty skipped), and shows up in admin notification bell + attendance list with reason.
- ✅ **Half-day leave / permission hours reduce the 8h requirement** (see previous entries — Emergency Clock-Out is the fallback for cases that couldn't be pre-planned).

### Sep 2026 (cont. 11)
- ✅ **Auto-end forgotten breaks**: If an employee forgets to click "End Break", the system now auto-closes it once it hits the `MAX_BREAK_MINUTES` (40 min) limit and resumes their working timer — no more getting stuck in break mode indefinitely. New `auto_close_expired_break()` helper is checked lazily inside `/attendance/status` (polled every 60s by `TimeTrackerCard.jsx` while the dashboard is open), caps the break at exactly 40 min (not whenever the poll happened), updates `total_break_minutes`, and emails `hr@sparkcurv.com` + `ponish.jino@sparkcurv.com` (`_break_auto_end_email_html`, mocked-SMTP pattern — same as other notify emails). Response includes a one-shot `break_auto_ended` flag; frontend shows a toast ("Your break was auto-ended after 40 minutes — working timer resumed.") only on the exact call it happens, not on subsequent polls. Verified via curl: break started 50 min ago → first `/attendance/status` call auto-closes it (`on_break:false`, `total_break_minutes:40`, `break_auto_ended:true`); second call confirms it doesn't re-fire (`break_auto_ended:false`).
- ✅ **Emergency Clock-Out** (see previous entry).
